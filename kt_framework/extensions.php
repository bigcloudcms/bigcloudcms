...
function kt_remove_sections() {
    ...
    if(isset($options_slug['bigcloudcms_woo_extension']) && $options_slug['bigcloudcms_woo_extension'] == '0') {
        //sections
        ...
        add_filter('bigcloudcms_widget_carousel_types', 'bigcloudcms_unset_product_from_carousel');
        function bigcloudcms_unset_product_from_carousel($types) {
    ...
    if(isset($options_slug['bigcloudcms_header_footer_extension']) && $options_slug['bigcloudcms_header_footer_extension'] == '1') {
        add_action('wp_head', 'kt_wp_head_script_output');
        function kt_wp_head_script_output() {
            global $bigcloudcms_premium;
            if(isset($bigcloudcms_premium['kt_header_script']) && !empty($bigcloudcms_premium['kt_header_script']) ){
                echo $bigcloudcms_premium['kt_header_script'];
            }
        }
        add_action('wp_footer', 'kt_wp_fotoer_script_output');
        function kt_wp_fotoer_script_output() {
            global $bigcloudcms_premium;
            if(isset($bigcloudcms_premium['kt_footer_script']) && !empty($bigcloudcms_premium['kt_footer_script']) ){
                echo $bigcloudcms_premium['kt_footer_script'];
            }
        }
    } else {
        if(isset($options_slug['bigcloudcms_header_footer_extension'])) {
            Redux::removeSection(OPTIONS_SLUG, 'header_footer_scripts');
        }
    }
    if(isset($options_slug['bigcloudcms_portfolio_extension']) && $options_slug['bigcloudcms_portfolio_extension'] == '0') {
        ...
        remove_action( 'init', 'bigcloudcms_portfolio_post_init', 1 );
        ...
        add_filter('bigcloudcms_widget_carousel_types', 'bigcloudcms_unset_portfolio_from_carousel');
        function bigcloudcms_unset_portfolio_from_carousel($types) {
            unset($types['portfolio']);
            return $types;
        }
        add_filter( 'bigcloudcms_portfolio_post_removal', 'bigcloudcms_portfolio_post_removal_function');
        function bigcloudcms_portfolio_post_removal_function() { 
            return false;
        }
    }
    if(isset($options_slug['bigcloudcms_staff_extension']) && $options_slug['bigcloudcms_staff_extension'] == '0') {
      remove_action( 'init', 'staff_post_init');
    }
    if(isset($options_slug['bigcloudcms_testimonial_extension']) && $options_slug['bigcloudcms_testimonial_extension'] == '0') {
      remove_action( 'init', 'testimonial_post_init');
        add_filter( 'bigcloudcms_testimonial_post_removal', 'bigcloudcms_testimonial_post_removal_function');
        function bigcloudcms_testimonial_post_removal_function() { 
            return false;
        }
        add_filter('bigcloudcms_shortcodes', 'bigcloudcms_unset_test_from_shortcode_popup');
        function bigcloudcms_unset_test_from_shortcode_popup($bigcloudcms_shortcodes) {
            unset($bigcloudcms_shortcodes['kad_testimonial_form']);
            return $bigcloudcms_shortcodes;
        }
    }
}


function bigcloudcms_portfolio_post_on() {
    return apply_filters( 'bigcloudcms_portfolio_post_removal', true );
}
function bigcloudcms_testimonial_post_on() {
    return apply_filters( 'bigcloudcms_testimonial_post_removal', true );
}