...
function bigcloudcms_seo_switch() {
  global $bigcloudcms_premium;
   if(isset($bigcloudcms_premium['seo_switch'])) {
      if($bigcloudcms_premium['seo_switch'] == 1 ) {
        $useseo = true;
      } else {
        $useseo = false;
      }
...
function bigcloudcms_display_sidebar() {
   if (class_exists('woocommerce'))  {
        $sidebar_config = new BigCloudCMS_Sidebar(
        array('bigcloudcms_sidebar_on_archive_page','bigcloudcms_sidebar_on_shop_page','bigcloudcms_sidebar_on_shop_cat_page','bigcloudcms_sidebar_on_blog_post','bigcloudcms_sidebar_on_blog_page','bigcloudcms_sidebar_on_product_post', 'bigcloudcms_sidebar_on_staff_post','is_404','bigcloudcms_sidebar_on_home_page','bigcloudcms_sidebar_on_myaccount_page','is_cart','is_checkout',array('is_singular', array('portfolio')), array('is_tax', array('portfolio-type')), array('is_tax', array('portfolio-tag'))
        ),
  ...
  if( is_product()) {
    ...
    $postsidebar = get_post_meta( $post->ID, '_kad_post_sidebar', true );
      if(isset($postsidebar) && $postsidebar == 'yes') {
        ...
        global $bigcloudcms_premium; 
        if(isset($bigcloudcms_premium['product_sidebar_default']) && $bigcloudcms_premium['product_sidebar_default'] == 'yes') {
            return false;
          } else  {
            return true;
          }
...
function kt_sub_indicator($classes) {
 global $bigcloudcms_premium;
  if(isset($bigcloudcms_premium['show_subindicator']) && $bigcloudcms_premium['show_subindicator'] == 1) {
