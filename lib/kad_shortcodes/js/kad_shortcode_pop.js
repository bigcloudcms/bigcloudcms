...
jQuery(document).ready(function($){
	...
	$('body').on('click','.bigcloudcms-generator-button',function(){
       
 
            $.magnificPopup.open({
                mainClass: 'mfp-zoom-in',
 	 		 	items: {
 	  	     		src: '#bigcloudcms-shortcode-innercontainer'
  	        	},
  	         	type: 'inline',
	    });         
	...
	$('#kad-shortcode-insert').click(function(){
		...
		var scname = $('#bigcloudcms-shortcodes').val();
		var output = '';
		if(scname == 'columns') {
				var columnssetup = '';
		...
		//reset data
		$('.bigcloudcmsshortcode-content').find('input:text, input:file, textarea').val('');
		$('.bigcloudcmsshortcode-content').find('select').removeAttr("selected", " ");
		$('.bigcloudcmsshortcode-content').find('input:radio, input:checkbox').removeAttr('checked').removeAttr('selected');
		$('.bigcloudcmsshortcode-content').find('.wp-color-result').attr('style','');
	}

		$('#bigcloudcms-shortcodes').change(function(){
